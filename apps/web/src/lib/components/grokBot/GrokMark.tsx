...
/**
 * Grok mark from svgl `grok-light.svg` / `grok-dark.svg`. Same `size` prop
 * shape as the Tabler icons in settings nav.
 */
export function GrokMark({
  ...
  return (
    <svg
      ...
      fill="none"
      className={cn("shrink-0 text-[#0A0A0A] dark:text-white", className)}
      aria-hidden="true"
    >
      <path fill="currentColor" d={GROK_PATH_A} />
      <path fill="currentColor" d={GROK_PATH_B} />
    </svg>
  );
}
