...
>(({ className, ...props }, ref) => {
  ...
  return (
    <button
      ref={ref}
      className={cn(
        "max-sm:hit-target absolute h-8 w-8 cursor-pointer rounded-full border bg-card/95 backdrop-blur-sm flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed hover:bg-muted motion-press active:scale-[0.94]",
        orientation === "horizontal"
    ...
    <div className={cn("flex justify-center gap-1.5 pt-2", className)}>
      {scrollSnaps.map((_, index) => (
        <button
          ...
          type="button"
          className={cn(
            // The dot's 10px width step is deliberately not animated: it is
            // below the threshold where the change reads as motion, and width
            // would relayout the whole dot row every frame.
            "h-1.5 cursor-pointer rounded-full motion-press active:scale-90",
            index === selectedIndex
              ? "w-4 bg-primary"
