...
/**
 * Builds every Eva MCP tool the caller is allowed to see. The result is mounted
 * either flat (one MCP tool each) or behind the code-mode `execute` tool.
 */
export function buildTools(
  ...
  tools.push(
    defineTool({
      name: "get_document",
      description: "Get a single document by its Convex document ID.",
      mutating: false,
      input: {
        id: z
          .string()
      ...
      handler: async ({ id, repoId, repoName, app, environment }) => {
  ...
  tools.push(
    defineTool({
      name: "run_query",
      description: `Run arbitrary read-only Convex query code against a repo's database. This is the most powerful tool — use it for joins, aggregations, filters, and complex data retrieval.
...
Example: "const users = await ctx.db.query('users').collect(); return users.filter(u => u.role === 'admin').length;"`,
  ...
  tools.push(
    defineTool({
      name: "count_table",
      description: "Count the total number of documents in a table.",
      mutating: false,
      input: {
        table: z.string().describe("Table name"),
        ...repoRefArgs,
      ...
      handler: async ({ table, repoId, repoName, app, environment }) => {
  ...
  tools.push(
    defineTool({
      name: "postgres_query",
      description: `Run read-only SQL against a repo's Postgres read replica (the POSTGRES_READ_REPLICA_URL env var configured for the repo in Eva).
...
For schema discovery, query information_schema (e.g. "SELECT table_name FROM information_schema.tables WHERE table_schema = 'public'").`,
      ...
      input: {
        sql: z
          .string()
          .describe("A single read-only SQL statement to execute."),
        limit: z
          .number()
          .max(1000)
          .default(100)
      ...
      handler: async ({ sql, limit, repoId, repoName, app }) => {
        const { userId } = await getContext();
        const ref = await resolveRepoRef({ repoId, repoName, app }, userId);
        if ("isError" in ref) return ref;
  ...
  tools.push(
    defineTool({
      ...
      description: `Create multiple tasks at once with dependencies between them, and optionally group them into a project.
...
This creates 3 tasks where Build API depends on Setup DB schema, and Build UI depends on Build API.`,
  ...
  tools.push(
    defineTool({
      ...
      description: `Send a chat message into an EXISTING Eva session, quick task or project and run it there, exactly as typing in that chat does. Use this to carry on with a pull request Eva already opened. It never creates a new session, task or project.
...
Sending wakes the chat's preview sandbox. Call stop_sandbox once you are done with it so the VM does not keep running.`,
  ...
  tools.push(
    defineTool({
      ...
      handler: async ({ repoName, title, content, app }) => {
        const { userId } = await getContext();
  ...
  tools.push(
    defineTool({
      ...
      handler: async ({ repoName, app, kind }) => {
        const { userId } = await getContext();
        const resolved = await resolveRepoByName(repoName, app, userId);
        if ("isError" in resolved) return resolved;
        const { repo } = resolved;
...
Do NOT use this for session walkthrough recordings, screen captures, or screenshots. For those, save the file under repo-root recordings/ or screenshots/ with agent-browser and leave it on disk — Eva attaches it to the chat message with the built-in video/image player.`,
      ...
      handler: async ({ name, html, description, teamId, declaredTools }) => {
        const { userId } = await getContext();
  ...
  tools.push(
    defineTool({
      ...
      handler: async ({ artifactId }) => {
        await getContext();
        const result = await ctx.runAction(
          internal.mcp.nodeActions.getArtifact,
          {
            clerkUserId,
            artifactId,
          },
